...
export function ParticleNetwork() {
  ...
  useEffect(() => {
    ...
    const nodeLabels = ["NODE 01", "NODE 02", "PROC", "DATA", "SYS", "CORE", "NET", "API"]
    const nodeCount = Math.min(8, Math.max(5, Math.floor(canvas.width / 300)))
    nodesRef.current = Array.from({ length: nodeCount }, (_, i) => ({
      x: (canvas.width / (nodeCount + 1)) * (i + 1) + (Math.random() - 0.5) * 100,
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (from !== to && !connectionsRef.current.some((c) => c.from === from && c.to === to)) {
        connectionsRef.current.push({
          from,
          to,
          progress: Math.random(),
    ...
    const animate = () => {
      ...
      connections.forEach((connection) => {
        ...
        const mouseDistToLine = pointToLineDistance(mouse.x, mouse.y, fromX, fromY, toX, toY)
        ...
        ctx.lineTo(toX, toY)
        ctx.strokeStyle = isNearMouse ? "rgba(100, 200, 255, 0.5)" : "rgba(150, 150, 150, 0.3)"
      ...
      nodes.forEach((node, i) => {
        ...
        // Draw label
        ctx.fillStyle = isNearMouse ? "rgba(100, 200, 255, 0.9)" : "rgba(200, 200, 200, 0.8)"
        ...
        ctx.fillText(`[${String(i).padStart(2, "0")}]`, node.x + node.width / 2, node.y + node.height / 2 + 8)
        ctx.fillText(
